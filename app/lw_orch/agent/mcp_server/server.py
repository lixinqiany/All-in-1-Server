...
def get_date_info(offset: int = 0) -> dict[str, Any]:
    ...
    You can get current time and date here.
    ...
    formatted_datetime = target_date.strftime("%Y年%m月%d日 %H:%M:%S")
...
@mcp.tool()
async def get_access_token(base_url: str, 
                           client_id: str, 
                     client_secret: str, 
    ...
    Args:
        base_url: base url of the LightWAN Orch Server
...
@mcp.tool()    
async def get_all_sites_info(base_url: str, 
                             scope: Literal["global", "customer"],
                             access_token: str, 
                             customer_id: str|None = None,
                             page: int|None = None, 
                             size: int = 20) -> dict[str, Any]:
    """Get all sites info for a given customer id. 
    This is a paginated query interface where you can control the page number and page size.
    
    Args:
        base_url: base url of the LightWAN Orch Server
        access_token: access token
        scope: scope of the access token, which can be "global" or "customer"
        customer_id: customer id if scope is "global"
    ...
    url_suffix = 'openapi/v2/sites' if scope == 'customer' else f'openapi/v2/customer/{customer_id}/sites'
    async with httpx.AsyncClient() as client:
        if page is None:
            # 全量查询：循环获取所有页面
            all_sites = []
...
if __name__ == "__main__":
    # fastmcp 文档是有错误的，没法再run里设置host和port参数，只能通过setting属性设置
